...
testthat::test_that("Utility function handle_url2 works", {
  ...
  testthat::expect_equal(err$message, "Must specify at least one of url or handle")

  # Test output - url provided
  result <- handle_url2(url = "https://api.sbgenomics.com/v2/user/", query = list(limit = 50, offset = 50))
  ...
  testthat::expect_equal(result$url, "https://api.sbgenomics.com/v2/user/?limit=50&offset=50")
...
testthat::test_that("Utility function build_url2 works", {
   ...
   testthat::expect_equal(resulting_url, "https://api.sbgenomics.com/v2/user/?limit=50&offset=0")
...
testthat::test_that("Utility function set_headers works when authorization = FALSE", {
  ...
  testthat::expect_equal(typeof(headers), "character",
                         label = glue::glue("Headers should be a vector of characters, not {typeof(headers)}."))
  testthat::expect_equal(length(headers),
                         3L, label = "Headers vector should have three elements: X-SBG-Auth-Token, Accept and Content-Type")
  testthat::expect_equal(names(headers), c("X-SBG-Auth-Token", "Accept", "Content-Type"),
                         label = "Elements in headers vector do not have expected names.")
  testthat::expect_equal(unname(headers), c(token, "application/json", "application/json"),
                         label = "Headers elements are not as-expected.")


})

testthat::test_that("Utility function set_headers works when authorization = FALSE and advance_access = TRUE", {
  ...
  testthat::expect_equal(typeof(headers), "character",
                         label = glue::glue("Headers should be a vector of characters, not {typeof(headers)}."))
  testthat::expect_equal(length(headers),
                         4L, label = "Headers vector should have three elements: X-SBG-Auth-Token, Accept, Content-Type and X-SBG-advance-access ")
  testthat::expect_equal(names(headers), c("X-SBG-Auth-Token", "Accept", "Content-Type", "X-SBG-advance-access"),
                         label = "Elements in headers vector do not have expected names.")
  testthat::expect_equal(unname(headers), c(token, "application/json", "application/json", "advance"),
                         label = "Headers elements are not as-expected.")
})


testthat::test_that("Utility function set_headers works when authorization = TRUE", {
...
testthat::test_that("Utility function set_headers throws an error if token is not provided", {
...
testthat::test_that("Utility function setup_query works", {
  ...
  query <- setup_query(query = query, limit = getOption("sevenbridges2")$limit, offset = getOption("sevenbridges2")$offset , fields = fields)
...
testthat::test_that("Utility function m.fun works", {
  ...
  term = "api"
  search_through_vector <- c("element 1", "element 2 api", "element 3",
                             "element 4 API", "element 5", "element 6", "api", "element 8")

  match_index_vector <- m.fun(x = term, y = search_through_vector, exact = FALSE, ignore.case = TRUE)

  testthat::expect_equal(match_index_vector, c(2, 4, 7)) # unnamed vector of match indexes

  # test output when exact parameter is FALSE, and ignore.case is FALSE
  match_index_vector <- m.fun(x = term, y = search_through_vector, exact = FALSE, ignore.case = FALSE)

  testthat::expect_equal(match_index_vector, c(2, 7)) # unnamed vector of match indexes

  # test output when exact parameter is TRUE, and ignore.case is TRUE
  match_index_vector <- m.fun(x = term, y = search_through_vector, exact = TRUE, ignore.case = TRUE)

  testthat::expect_equal(match_index_vector, c(7)) # named vector

  # test output when exact parameter is FALSE, and ignore.case is TRUE and there is only one match
  term = "element 1"

  match_index_vector <- m.fun(x = term, y = search_through_vector, exact = FALSE, ignore.case = TRUE)

  testthat::expect_equal(match_index_vector, c(`element 1` = 1)) # named vector of match indexes
})


testthat::test_that("Utility function m.match works", {

  # Test output when exact parameter is FALSE, ignore.case is TRUE (id = NULL and name != NULL)
  search_through_list <- list(list(name = "project 1", id = "asdf1234"),
  ...
  matchings <- m.match(obj = search_through_list, id = NULL, name = "project 3", exact = FALSE, ignore.case = TRUE)
  ...
  matchings <- m.match(obj = search_through_list, id = "xyzq2234", name = NULL, exact = FALSE, ignore.case = TRUE)
  ...
  # Test output when exact parameter is FALSE, and ignore.case is FALSE (no matchings)
  matchings <- m.match(obj = search_through_list, id = NULL, name = "PROJECT 3", exact = FALSE, ignore.case = FALSE)
