test_that("User print method works", {
  user_obj_file <- testthat::test_path(
    "test_data",
    "luna_lovegood_user_obj.RDS"
  )
...
test_that("User initialization works", {
  ...
  test_user_response <- readRDS(testthat::test_path(
    "test_data",
    "luna_resp.RDS"
  ))
  ...
  testthat::expect_true(checkmate::test_class(test_user,
    classes =
      c("User", "Item", "R6")
  ))
  ...
  testthat::expect_equal(
    test_user$href,
    "https://api.sbgenomics.com/v2/users/luna_lovegood"
  )
