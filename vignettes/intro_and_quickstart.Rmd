...
There we have groups of endpoints for: projects, files, apps, tasks, invoices, 
volumes etc.
...
as an input to your task. You can find a resource's ID in the final part of 
the URL on the visual interface or via the API requests to list resources or 
...
b <- sevenbridges2::Auth$new(from = "file", profile_name = "aws-us-<username>")
```
...
Please refer to the [API reference section](#auth-reference) for more usage and 
technical details about the three authentication methods.
...
[Billing group and Invoices section](#billing_invoices).
...
- Browse them online. Check out the [tutorial](https://docs.sevenbridges.com/docs/search-for-an-app) in the 
"Find apps" section.
...
# Search by ID
star_app <- a$apps$get(
  id = "admin/sbg-public-data/rna-seq-alignment-star-2-5-4b/25"
)
```
...
`admin/sbg-public-data/rna-seq-alignment-star-star-2-5-4b/25` into your project, 
...
reads_1 <- a$files$get(id = "641c48c425ed1842bd0bf799")
reads_1$copy_to(project = p)

reads_2 <- a$files$get(id = "641c48c425ed1842bd0bf835")
reads_2$copy_to(project = p)

# Get a single file reference file and copy into a project
fasta_in <- a$files$get(id = "641c48c525ed1842bd0bf86a")
fasta_in$copy_to(project = p)

# Get gtf file and copy into a project
gtf_in <- a$files$get(id = "641c48c425ed1842bd0bf825")
...
```{r}
# Add new tasks
taskName <- paste0("TestUser-star-alignment ", date())
...
a$files$copy(
  ...
  destination_project = "<short-name-of-the-project>"
)
```
