...
`Important note:` This is a work-in-progress project to update the `sevenbridges2` package. Accordingly, this vignette will also change as new features are implemented.

# Introduction

`sevenbridges2` is an R package that provides an interface for Seven Bridges public API. The supported platforms includes the [Seven Bridges Platform](https://igor.sbgenomics.com/), [Cancer Genomics Cloud (CGC)](https://www.cancergenomicscloud.org), [BioData Catalyst (BDC)](https://platform.sb.biodatacatalyst.nhlbi.nih.gov/) and [Cavatica](https://cavatica.sbgenomics.com).

Learn more from our documentation on the [Seven Bridges Platform](https://docs.sevenbridges.com/page/api), the [Cancer Genomics Cloud (CGC)](http://docs.cancergenomicscloud.org/v1.0/page/the-cgc-api), 
...
***(Advanced user option)*** The second way of constructing an API request is to directly use the `httr` package to make your API calls.


## API General Information

The `sevenbridges2` package is organized by main resources from the Seven Bridges API reference. 
There we have groups of endpoints for: projects, files, apps, tasks, invoices, volumes etc.
For each group of resources the set of operations is available like `query()`, `get()` and `delete()` as common and other custom operations. 
...
Every API call that returns as a result some list of items (usually the output from 
`query()` operations), like fetching projects, files, apps etc, wraps the results into a general `Collection` class object containing `items` field from which users may access the items returned. 
Additional options that `Collection` class is offering are possibilities to 
navigate between pages of results, for example, to load next or previous page of results 
by calling methods `next_page()` and `prev_page()`.

Lastly, users are able to fetch all results using `Collection`'s method `all()` which is a 
shortcut to send multiple API calls for each next page and collect all results. 
Keep in mind the limit used as well as the API rate limit.
...
By default, `offset` is set to `0` and `limit` is set to `50`. As such, your API request returns the __first 50 items__ when you list items or search for items by name. To search and list all items, use `complete = TRUE` if you use core `api()` function in your API request, or `all()`
operation within the Collection object you've received as result. 


__Search by ID__

When searching by ID (usually it's the resource's `get()` operation), your request will return your exact resource as it is unique. As such, you do not have to set `offset` and `limit` manually. It is a good practice to find your resources by their ID and pass this ID as an input to your task. You can find a resource's ID in the final part of the URL on the visual interface or via the API requests to list resources or get a resource's details.
...
Read more about billing and invoices operations in the [Billing group and Invoices section](#billing_invoices).
