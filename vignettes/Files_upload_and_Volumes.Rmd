---
title: "Files upload and Volumes"
date: "`r Sys.Date()`"
output:
...
vignette: >
  ...
  %\VignetteIndexEntry{Files upload and Volumes}
...
to upload your files into the Platform.
...
This upload job is wrapped into an object of class `Upload` where you can
see its details and call other actions on it. 

Let's initialize upload job that will upload file into a project:
```{r}
# Authenticate
a <- Auth$new(platform = "aws-us", token = "<your-token>")

# Get desired project to upload to
...
If you would like to upload your file into some folder, you need to set
`parent` parameter:
...
First, let's print the `Upload` object to see what API returned as response.
```{r}
up$print()
```
...
In previous example we can see that API returned the upload id and some 
information about sizes. 
First we see the `file_size` in bytes (232), which is the real size of the file. 
Uploading files actually in the background splits the files into parts which
are then being uploaded one by one or in parallel and then united again on 
destination.
Each part can weight 5GB maximum, while the default `part_size` is recommended
and set to be 32MB (which is 33554432B in our example). 

Lastly, number of parts or `part_length` field, is also an important measure. 
Maximum number of parts can be 10.000. 

Since users can control part size through parameter `part_size` in `upload()`
function, they should be careful to not set this size too small for very large 
files, so that total number of parts doesn’t exceed the limit of maximum 10000.

<div align="right"><a href="#top">top</a></div>

### Start upload

Call `start()` method on the upload job object do start the upload process.
...
`FALSE` when creating upload job and the upload process will start right away.
...
In the result beside basic information, you will receive the info on the
...
Going back to authentication object, there are two more operations for uploads
manipulation. 
...
**Note** that in practice, if you start some big upload job, your R session 
...
terminology. At a certain level, though, they all view resources as data objects organized in repositories. Authentication and operations are commonly defined 
on those objects and repositories, and while each cloud provider might call 
these things different names and apply different parameters to them, their basic behavior is the same.
...
Currently, volumes may be created using two types of cloud storage repository: 
...
external storage. You can 'import' files from the volume to your Seven Bridges environment to use them as inputs for computation. Similarly, you can write 
...
All volume related operations for querying volumes, fetching single volume, 
creating and creating volumes are grouped under `volumes` path (`Volumes` 
resource class) on the authentication object. 

When operating with single volume, it is represented as object of class `Volume`
which stores all volume information returned from API and additional methods you
are able to call directly on the volume, like volume update, deactivation, 
listing content, volume members manipulation etc.
...
Enterprise users, while for public program users it can be in form 
...
* `get_member`: get some volume member information
...
Same as we had with `Collection` objects, here we also have pagination
functions to return next page of results or all results. However, backwards
pagination is not available for volume's contents. 

Users can navigate through pages of results also by using `continuation token` 
...
User can remove volume members by providing its username or object of the 
`Member` class to the `remove_member()` function:
```{r}
# Remove member
...
Function for adding new members to the volume can accept a `Member` object 
(for example used in some project) or its username. 
```{r}
...
Creating and connecting volumes to the Platform allow you to import your 
files/folders from some cloud bucket into the Platform. 
Imports operations are related to volumes, but in the API they are 
separated under `/imports` endpoints, so in our library they are also grouped
under `imports` path on the authentication object (`Imports` resource class). 

Single import job is represented as `Import` class object containing information
about which file/folder has been or is being imported, from which volume, 
to which project/folder on the Platform, time when started and finished, 
status of the job, logs etc. 

## List volume imports

To preview and query all import jobs you've created use `query()` function on
`Auth$imports` path:
...
In order to import volume files into the project, users can use 
`submit_import()` method from `Auth$imports` path, or directly on the selected `VolumeFile` object (file they want to import) where this function is also 
available. 
...
Preview import job details with `print()` method:
```{r}
# Print Import object
print(imp_job1)
```

You can also import folders from the volume into the project, with option to 
...
Exports state for the actions of exporting your files on Platform into some 
cloud bucket represented via volumes.
...
under `exports` path on the authentication object (`Exports` resource class). 

Single export job is represented as `Export` class object containing information
about which file has been or is being exported, from which project/folder
on the Platform, to which volume, time when started and finished, status of the 
job, logs etc. 

## List file exports into the volumes

Users are able to preview and query all export jobs they've created when they 
were trying to export their files from the Platform into some cloud bucket using volumes. 
The output is a `Collection` object storing list of exports in its `items`
field and with pagination options.
...
# List exports into specific volume
...
results contains all files exported to that specific volume they're being 
called from.
...
# Get a File object/id you want to export from the platform
...
Preview export job details with `print()` method:
```{r}
# Print export job info
print(exp_job1)
```

Bear in mind that **folders export** from the platform into the volumes is 
**not possible** with this function. For such cases (or multiple files export) 
it is better to use bulk actions that will be added soon into the package.

Users can also export the file into some volume's directory, by providing the 
`prefix` within the `location` parameter as a folder name, that will be 
...
the newly exported object on the volume. The source file will be deleted from the Seven Bridges environment and, if no more copies of this file exist, it will no 
longer count towards your total storage price on the Seven Bridges environment.
