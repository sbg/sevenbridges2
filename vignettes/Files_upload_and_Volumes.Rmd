...
Users are able to fetch details for multiple import jobs by calling one bulk action - `bulk_get()` method. The accepted input can be a list of
import job IDs or a list of import job objects (of class Import).
The result will be a Collection object containing list of (updated) 
import jobs.

```{r}
# Get multiple import jobs details
...
## Submit bulk import - import multiple volume files or folders into the project

Users are able to perform bulk action under multiple volume files or folders and
import them into a project using a single call of the method 
`bulk_submit_import()` from the `Auth$imports` path.

The required input should be a nested list of elements for each file/folder you
want to import containing specific fields: `source_volume`, `source_location`, 
`destination_project`, `destination_parent`, `name`, `autorename` and 
...
# list of import jobs created in previous step
a$imports$bulk_get(imports = bulk_import_jobs$items)
```

As you may see from the example above, users are able to import folders from the 
volume into the project or some project's directory, with the option to 
preserve or not to preserve folder structure.

Moreover, you are able to pass the objects of classes Volume, Project or File 
(with type = 'folder') for `source_volume`, `destination_project` and 
`destination_parent` fields when constructing the inputs list, besides using 
their string IDs.


In the example above, the list of items for a bulk import job was manually
...
provided `VolumeFile` or `VolumePrefix` objects, filling the following fields
for each item: `source_volume`, `source_location`, `destination_project` or
`destination_parent`, `autorename`, `preserve_folder_structure`.

Note that the same `destination_project`/`destination_parent` and `autorename`
values will be applied uniformly across all items in the resulting list.
The `preserve_folder_structure` parameter, if provided, applies exclusively to
`VolumePrefix` items.
...
Keep in mind that `prepare_items_for_bulk_import()` is designed solely to
...
Users are able to fetch details for multiple export jobs by calling one bulk action - `bulk_get()` method. The accepted input can be a list of
export job IDs or a list of export job objects (of class Export).
The result will be a Collection object containing list of (updated) 
export jobs.

```{r}
# Get multiple export jobs details
...
## Submit bulk export - export multiple files into the volume

Users are able to perform bulk action under multiple project files and
export them into a volume using a single call of the method 
`bulk_submit_export()` from the `Auth$exports` path.

The required input should be a nested list of elements for each file you
want to export containing specific fields: `source_file`, `destination_volume`, 
`destination_location`, `overwrite` and 
`properties` that accepts list with fields: `sse_algorithm`, 
`sse_aws_kms_key_id` and/or `aws_canned_acl`.
...
## Choose first 3 files to export
...
# Get updated status by fetching details with bulk_get() and by passing the
# list of export jobs created in previous step
a$exports$bulk_get(exports = bulk_export_jobs$items)
```

As you may see from the example above, users are able to export files into some 
volume's folder.

Moreover, you are able to pass the objects of classes Volume or File 
(with type = 'file' only, since folders are not possible to be exported) when 
constructing the inputs list, besides using their string IDs for `source_file` 
and `destination_volume` fields. However, `destination_location` on the volume 
must be set as character field. 

Lastly, `copy_only` field will apply to all files being exported, meaning, each 
file would be copied to the volume, while source location would remain on the 
Platform if `copy_only` is set to TRUE.

In the example above, the list of items for a bulk export job was manually
...
provided `File` objects, filling the following fields for each item: 
`source_file`, `destination_volume`, `destination_location`, `overwrite`,
`properties`.

Check function's documentation and [API documentation](https://docs.sevenbridges.com/reference/start-a-bulk-export-job)
for more details.
...
Keep in mind that `prepare_items_for_bulk_export()` is designed solely to
