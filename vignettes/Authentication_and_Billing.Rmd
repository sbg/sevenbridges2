---
title: "Authentication and Billing for Seven Bridges API R Client"
date: "`r Sys.Date()`"
output:
...
vignette: >
  ...
  %\VignetteIndexEntry{Authentication and Billing for Seven Bridges API R Client}
...
[Cancer Genomics Cloud](http://docs.cancergenomicscloud.org/v1.0/docs/get-your-authentication-token),
...
- The platform name you are using. This is an optional field, as the base URL 
of the API ultimately decides where the API calls will be sent to. 
...
1. The package will use the direct authentication method if `from` parameter is 
not specified explicitly or specified as `from = "direct"`.
...
In this example, the `url` points to the Seven Bridges Platform on 
...
The above will use the Seven Bridges Platform on AWS environment 
since no `platform` nor `url` were explicitly specified.
...
If you did not pass any parameters to `api()` from `Auth` object, it would 
...
since the `Auth` object already handles these information. 
...
which can enable the access to the "Advance Access" features in the 
Seven Bridges API. 
...
This offers a one-button switch for users who want to experiment with the 
...
## [1] FALSE
```
For example, the Markers API is in Advance Access as of November 2018. 
If we try to use the Markers API to list markers available on a BAM file with 
the `advance_access` option disabled, it will return an error message:
...
to return the fields id, name and size for files in a project, you may issue 
the call `p$list_files(fields = c("id" ,"name", "size"))`.

3. The fields parameter can be used to exclude a specific file: if you wish to 
omit certain field from the response, do so using the fields parameter with 
...
hour, along side with time when will rate limit reset and total and remaining
...
This call will return a successful response of currently authenticated user, 
while if you have some advanced privileges as Enterprise user, you can 
...
from here you can query all billing groups or fetch single one by its id. 

When you fetch single billing group it will be an object of class `Billing`. 
...
Same logic applies for the invoices - the operations available for invoices are
grouped under Invoices class and from here you can query of fetch single invoice
with resource operations `query()` and `get()`.
...
To send feedback to Velsera via the API, you can use the `Auth` class method 
`send_feedback()`. 
...
when the required break time exceeds,
-maintenance sleeper which pauses the execution if SevenBridges API is 
