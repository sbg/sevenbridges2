...
Moreover, users are able to fetch all results using `Collection`'s method 
`all()` which is a shortcut to send multiple API calls for each next page and 
collect all results. Keep in mind the `limit` used as well as the API rate limit.

Lastly, printing `Collection` objects will print first 10 items (if there's more 
than 10 items in results) by default, but this can be changed with `n` 
parameter in its `print()` function: 

```{r}
# Create some collection of files
...
the \code{billing_group_id} query parameter to specify the ID of a particular 
