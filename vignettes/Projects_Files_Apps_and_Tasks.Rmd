...
a$projects$delete(project = "<project_id>")
...
api_testing_files <- a$files$query(project = "<username>/api-testing")
api_testing_files
```
```
[[1]]
...
To list publicly available files on Seven Bridges Platform, set the project
...
`Auth$files` path and accepts the ID of the file you want to delete. 

```{r}
# Delete file by id
a$files$delete(id = "<file_id>")
```
...
a$files$copy(
  ...
  destination_project = "<username>/api-testing"
)
```
...
my_project <- a$projects$get(project = "<username>/api-testing")
...
demo_file$copy_to(
  project = "<destination_project_object>",
  name = "<new_name>"
)
```
...
demo_file$move_to_folder(parent = "<parent_id>", name = "Moved_file.txt")
```
...
```{r}
# Get project
p <- a$projects$get("<username>/api-testing")
...
• name: BCFtools Call
• project: <username>/api-testing
• id: <username>/api-testing/new_app_json
• href: https://api.sbgenomics.com/v2/apps/<username>/api-testing/new_app_json/0
```
<div align="right"><a href="#top">top</a></div>
...
```{r}
my_app <- a$apps$get(id = "<username>/api-testing/new_app_json/0")
my_app$print()
```
...
• project: <username>/api-testing
• id: <username>/api-testing/new_app_json
• href: https://api.sbgenomics.com/v2/apps/<username>/api-testing/new_app_json/1
```
<div align="right"><a href="#top">top</a></div>
...
• created_by: <username>
• app: <username>/api-testing/rna-seq-alignment-star/0
• project: <username>/api-testing
