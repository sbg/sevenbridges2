...
a <- Auth$new(token = "<your token>", platform = "aws-us")
...
Within the authentication object, there is also a function for fetching a single
File object by its id, which is suitable if you know the id of a file of interest.
You can see the file's id on the Platform, when navigating to that file and
looking into the url. The url should be in form: 
`<platform_url>/u/<username>/<project_name>/files/<file_id>/`
and you can see it end with file's id you can use in the API.  

```{r}
a$get_file(id = "file_id")
```

## Copy files

This method allows you to copy multiple files into some project's root directory.
```{r}
a$copy_files(files = list())
```
