...
a$app$query(visibility = "public", limit = 10)
```
...
a$app$query(project = "<project_object_or_its_ID>", limit = 10)
```
...
a$app$query(visibility = "public", query_terms = list("VCFtools"), limit = 10)
```
...
```{r}
# List files in project root directory
a$app$query(visibility = "public", id = "admin/sbg-public-data/vcftools-convert")
```
...
bcftools_app <- a$app$get(id = "admin/sbg-public-data/bcftools-call-1-15-1")
```
...
app_copy <- a$app$copy(bcftools_app,
...
```{r}
copied_app <- my_app$copy(project = "<destination_project_object_or_its_ID>",
                          name = "new_app_name")
```
