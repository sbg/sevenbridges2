...
* `list_files` : list volume content
...
returned. 
The output is a `VolumeFilesCollection` collection object, that contains the 
information about the `continuation token` that can be used to fetch next chunk 
of results. 
Alternatively, you can use the `link` parameter for this too. 
...
files_collection <- aws_iam_user_volume$list_files(limit = 20)

# Print collection
files_collection

# List all files specific directory on the bucket
folder_files_collection <- aws_iam_user_volume$list_files(parent = "dir_name")

# Get next group of results by setting the continuation token
files_collection <- aws_iam_user_volume$list_files(
  limit = 20,
  continuation_token = "continuation_token"
)

# Get next group of results by setting the link parameter
aws_iam_user_volume$list_files(link = "link_to_next_results")

# Or use VolumeFileCollection's object method next_page() for this:
files_collection$next_page()
```
...
(`file_location)`, or a link to that file resource. The link is a `href` field 
of desired file received from response when returning list of volume files 
with `list_files()`.
...
aws_iam_user_volume$get_file(file_location = "file_location_on_bucket")

# Get single volume file info - by setting link
aws_iam_user_volume$get_file(link = "full/request/link/to/file")
```
...
imp_proj <- auth$imports$query(project = "my-project-id")
```
...
test_proj <- auth$get("my-project-id")
...
```{r}
# Get single import
imp_obj <- auth$imports$get(id = "import-job-id")
```
...
vol1_content <- vol1$list_files()

## select one of the volume files
volume_file_import <- vol1_content[[3]]

## make file import
imp_job1 <- auth$imports$submit_import(
...
volume_folder_import <- vol1_content[[4]]

# print import job info
print(imp_job1)

## make folder import
imp_job2 <- auth$imports$submit_import(
...
```{r}
# a better way
p <- a$projects(query = list(name = "demo"))
p$list_tasks()
...
## Create Batch task
To run tasks in batch mode we have `batch` and `batch_input` parameters.
First one is boolean and there we define whether to run batch task or not, and
second one defines input on which the task will be batched.
