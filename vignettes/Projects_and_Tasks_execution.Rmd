...
IDs or `File` objects. Non-empty folders are allowed to be deleted together 
with their content.
...
### Asynchronous copy action on multiple files 
...
This operation allows you to perform bulk copy action and to continue your work 
in R session. The status of the copy operation can be previewed by reloading
...
The `async_list_file_jobs()` method allows you to retrieve details all 
asynchronous file jobs (async_copy, async_delete, async_move) in a single 
API call. This method accepts `limit` and `offset` arguments to control the 
number of results returned. The output is a `Collection` object containing a 
list of those `AsyncJob` objects with details as their type, id, status, 
when they started/finished etc.
