...
Usually, tasks need at least the inputs that are marked as required by the app 
to be set. Information about which inputs are required or optional for an app 
is stored in its CWL. However, we have provided a utility function 
`input_matrix()` that can parse this information and return the app input 
matrix for you. This way, you will know how to construct the list of inputs 
(how to name them and make them available within Files) when creating the task.

[PLACEHOLDER for example]

Similar, there is another utility operation to list the expected outputs of an
