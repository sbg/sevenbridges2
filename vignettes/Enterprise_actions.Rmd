...
We have created the `Teams` class as `Resource` class, the same we have with
`Apps`, `Files`, `Projects` etc, beside the `Team` class that stores information
about a single team. 
...
Creating new teams (`create()`) requires only a `division` parameter where to 
create a team and a `name` parameter how to name a new team created. 
...
# Delete a team
a$teams$delete(team = "my-new-team")
```
