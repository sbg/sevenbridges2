  ...
  public = list(
    ...
    print = function() {
      ...
      if (!is.null(x$settings) && length(x$settings) != 0) {
        ...
        string_project_settings <- glue::glue("{names(project_settings)}: {project_settings}")
      }
      if (!is.null(x$tags) && length(x$tags) != 0) {
        project_tags <- x$tags
        names(project_tags) <- paste0("tag_", seq_along(project_tags))
        string_project_tags <- glue::glue("{names(project_tags)}: {project_tags}")
      }
      if (!is.null(x$permissions) && length(x$permissions) != 0) {
        project_permissions <- x$permissions
        string_project_permissions <- glue::glue("{names(project_permissions)}: {project_permissions}")
      }
    ...
    edit = function(name = NULL,
      ...
      if (self$permissions$write != TRUE) {
        ...
        req <- sevenbridges2::api(path = paste0("projects/", self$id),
                                  method = "PATCH",
                                  body = body,
    ...
    delete = function(){
      ...
      # user_input <- readline(glue::glue_col("Are you sure you want to delete {green {self$id}} project? (y/n) \n Selection: "))
      # if(user_input != 'y') rlang::abort('Exiting since you did not press y.')

      res <- sevenbridges2::api(path = paste0("projects/", self$id),
                                method = "DELETE",
                                token = self$auth$get_token(),
                                base_url = self$auth$url)
    ...
    member_list = function(pid = self$id, ...) {
      res <- sevenbridges2::api(path = paste0("projects/", pid, "/members"),
                                method = "GET",
                                token = self$auth$get_token(),
                                base_url = self$auth$url,
                                ...)
    ...
    ...) {
      ...
      if (!is.null(username) || !is.null(email)) {
        ...
        req <- sevenbridges2::api(path = paste0("projects/", pid, "/members"),
                                  method = "POST",
                                  token = self$auth$get_token(),
                                  body = body,
        ...
        rlang::abort("Neither username nor email are provided. You must provide at least one of these parameters before you can add a user to a project.")
      )
    ...
    member_delete = function(pid = self$id, username = NULL) {
      if (!is.null(username)) {
        req <- sevenbridges2::api(path = paste0("projects/", pid, "/members", "/", username),
                                  method = "DELETE",
                                  token = self$auth$get_token(),
                                  authorization = self$auth$authorization,
                                  base_url = self$auth$url)
        ...
        if (req$status_code == 204) {
          rlang::inform(message = glue::glue_col("User {green {username}} has been deleted from the {green {self$id}} project.", .literal = TRUE))
        }

      } else {
        rlang::abort("Please provide a username for the user you want to remove from the project.")
      }
    },
    ...
    member_permissions_modify = function(pid = self$id,
      ...
      if (!is.null(username)) {
        ...
        req <- sevenbridges2::api(path = paste0("projects/", pid, "/members", "/" , username, "/permissions"),
                                  method = "PATCH",
                                  token = self$auth$get_token(),
                                  body = body,
                                  authorization = self$auth$authorization,
