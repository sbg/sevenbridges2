...
Project <- R6::R6Class(
  ...
  public = list(
    ...
    #'      default.
    #'    \item `use_memoization` - Set to `FALSE` by default. If set to `TRUE`
    #'      [memoization](https://docs.sevenbridges.com/docs/about-memoization)
    #'      is enabled.
    #'    \item `use_elastic_disk` - If set to `TRUE`
    #'      [Elastic disk](https://docs.sevenbridges.com/page/elastic-disk) is
    #'      enabled.
    #'    \item `intermediate_files` (list) - Contains the following subfields:
    #'      \itemize{
    ...
    #'        \item `duration` - Specifies intermediate files retention period
    ...
    detailed_print = function() {
      ...
      if (!is.null(x$settings) && length(x$settings) != 0) {
        ...
        string_project_settings <-
          glue::glue("{names(project_settings)}: {project_settings}")
      }
      if (!is.null(x$tags) && length(x$tags) != 0) {
        ...
        string_project_tags <-
          glue::glue("{names(project_tags)}: {project_tags}")
      }
    ...
    #'  `QUEUED`, `DRAFT`, `RUNNING`, `COMPLETED`, `ABORTED`, `FAILED`.
